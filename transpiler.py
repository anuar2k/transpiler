...
def generate_ssa(code, func_idx, module, functions=None):
    ...
    for i in range(len(func_type.params)):
    ...
    # Declare locals
    for i in range(len(code.locals)):
        local_type = code.locals[i]
        if local_type == 'i32' or local_type == 'i64':
            lines.append(f"l{i} = 0  # {local_type}")
        else:
            lines.append(f"l{i} = 0.0  # {local_type}")
...
def _generate_ssa_instruction(instr, stack, var_counter, func_type, functions, module):
    ...
    # Variables
        ...
        var_idx = var_counter[0]
        var_counter[0] += 1
        lines.append(f"_{var_idx} = l{local_idx}")
        stack.append(var_idx)
        ...
        if stack:
            val_idx = stack.pop()
            lines.append(f"l{local_idx} = _{val_idx}")
